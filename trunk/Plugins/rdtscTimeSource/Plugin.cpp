...
extern "C" int plgInitialize(Engine* root)
{
	...
	mOldTimeSource = mRoot->getClock()->getTimeSource();
	...
	mRoot->getClock()->setTimeSource(mTimeSource);

	// declare new function to the scripting engine
	//std::vector<ScriptParam> param;
	//param.push_back(mCpu.get());
	ScriptEngine::GetSingleton().add("getCpuString", ScriptFunctionName(getCpuString, Cpu), VarArg(mCpu.get()));
...
extern "C" void plgRelease( void )
{
	...
	mRoot->getClock()->setTimeSource(mOldTimeSource);
