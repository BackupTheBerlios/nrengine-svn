...
namespace nrEngine{
	...
	* \ingroup helpers
	**/
	void _NRExport NR_sleep( uint32 milliseconds );
	...
	* \ingroup helpers
	**/
	::std::string _NRExport NR_convertVersionToString(uint32 version);
	...
	* \ingroup helpers
	**/
	uint32 _NRExport NR_createVersionInteger(uint8 major, uint8 minor, uint8 patch);
	 ...
	 * \ingroup hepers
	 **/
	std::string _NRExport NR_trim(const std::string& str);
